...
public class OALGestureListener extends SimpleOnGestureListener {
    ...
    public OALGestureListener(Context ctx) {
        ...
        VRLog.d("TAG", ">>>OALGestureListener screen size= (" + mScreenWidth + "x" + mScreenHeight);
    }
    ...
    public boolean onDoubleTap(MotionEvent e) {
        VRLog.d(TAG, ">>>onDoubleTap START");
        dismissReminder();
        VRLog.d(TAG, ">>>onDoubleTap END");
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        VRLog.d(TAG,">>>onFling ScreenWidth= " + mScreenWidth
        ...
        if (e1.getY() >= (mScreenHeight - 500)
                ...
                && e1.getY() > e2.getY()) {
            VRLog.d(TAG, ">>>onFling show setting");
            if (mOpenSettingPanel != null) {
                mOpenSettingPanel.onOpenSettingPanel();
            }
        } else {
            VRLog.d(TAG, ">>>onFling dismiss reminder");
            dismissReminder();
        }
